...
// Sort each category
_.each(categories, (functionList, name) => {
    categories[name] = _.sortBy(functionList, "name");
});
...
const functions = {
    createInstance: (definition, options) => {
        ...
        const instance = {
            definition, parameters: [], added: undefined,
        };
    ...
    updateParameter: (instance, strValue, updateIndex) => {
    ...
    queryString: (wfFunction, baseExpression) => {
        const qString = wfFunction.definition.name + "(";

        // Process parameters
        let parameters = _.map(wfFunction.parameters, (value, paramIndex) => {
            const type = wfFunction.definition.parameters[paramIndex].type;
        ...
        parameters = _.filter(parameters, (value, pIndex) => {
        ...
        if (baseExpression) {
            let baseIndex = _.has(wfFunction.definition, "baseIndex") ? wfFunction.definition.baseIndex : parameters.length;
